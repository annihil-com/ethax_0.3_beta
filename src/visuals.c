...
vec4_t gameBackColor = { 0.0, 0.0, 0.0, 0.95 };
vec4_t gameBorderColor = { 0.255f, 0.255f, 0.0f, 0.5f };
vec4_t gameFontColor = { 0.0f, 0.2f, 0.0f, 1.0f };
...
void drawAdvert() {
	if( eth.isMenuOpen )
		return;	
	#define ADVERT_SIZE 0.30f
	#define ADVERT_Y 40
	char str[] = "EThax :" ETH_VERSION "*" ;
	//create window
...
void drawEspEntities() {
	...
	for (entityNum = 0; entityNum < MAX_CLIENTS; entityNum++) {
		...
		if (seth.value[VAR_ESP_FADE] == ESP_FADE_AUTO) {
			...
			fadeColor = max(fadeColor, 0.1f);	// Max fade
			fadeColor = min(fadeColor, 1.8f);	// Min fade
		// Manual fade
		} else {
		...
		if( seth.value[VAR_ESPNAME] == NAMES_EVERYBODY ||
			...
			// Distance based font size
				...
				fontScale = max(fontScale, 0.13f);	// Min size
				fontScale = min(fontScale, 0.27f);	// Max size
			}
	...
	for (entityNum = MAX_CLIENTS; entityNum < MAX_GENTITIES && seth.value[VAR_ESPENT]; entityNum++) {
		...
		if(IS_LANDMINE(entityNum) && IS_MISSILE_ARMED(entityNum)) {
			drawText(entity->screenX - 4 , entity->screenY, 0.18f, "Mine", colorGreen, qfalse, qtrue, &eth.fontFixed);
		// Dynamite esp
			...
			if (time > 0) {
				snprintf(timeStr, sizeof(timeStr), "TnT:%i", time);
				drawText(entity->screenX - 16, entity->screenY, 0.18f, timeStr, colorRed,qfalse, qtrue, &eth.fontFixed);
			} else
			drawText(entity->screenX - 4, entity->screenY, 0.13f, "TnT", colorGreen, qfalse, qtrue, &eth.fontFixed);
		// Grenade esp
		} else if (IS_GRENADE(entityNum)) {
			drawText(entity->screenX - 4, entity->screenY, 0.18f, "Nade", colorRed, qfalse, qtrue, &eth.fontFixed);
		// Satchel esp
		} else if (IS_SATCHEL(entityNum)) {
			drawText(entity->screenX - 4, entity->screenY, 0.18f, "Satchel", colorBlue, qfalse, qtrue, &eth.fontFixed);
		// Panzer esp
		} else if (IS_PANZER(entityNum)) {
			drawText(entity->screenX - 4, entity->screenY, 0.18f, "Panzer", colorCyan, qfalse, qtrue, &eth.fontFixed);
		// smoke esp
		} else if (IS_SMOKE(entityNum)) {
			drawText(entity->screenX - 4, entity->screenY, 0.18f, "Smoke", colorBlack, qfalse, qtrue, &eth.fontFixed);
		}
	}
	// World entities esp
		...
		for (entityNum = 0; entityNum < eth.refEntitiesCount; entityNum++) {
			...
			if (worldToScreen(refEnt->origin, &x, &y)) {
				...
				if (refEnt->hModel == eth.hAmmoPack) {
					drawText(x - 4, y, 0.13f, "Ammo", colorGreen, qfalse, qtrue, &eth.fontFixed);
				// MedicPack esp
				} else if (refEnt->hModel == eth.hMedicPack) {
					drawText(x - 4, y, 0.40f, "+", colorRed, qfalse, qtrue, &eth.fontFixed);
				}
				}
			}
		}
...
void drawSpawnTimerRight() {
    ...
    int w = sizeTextWidth(0.22f, str, &eth.fontVariable);
    int y = 136 - 14;
    int x = 634 - w;
...
void drawETHhud() {
	...
	// Immortal Add this fature to menu can be turn on/off
    if ((seth.value[VAR_RELOAD_TXT]) && eth.reloading && eth.reloadScale) {
	char relStr[10];
	...
	drawText(eth.cg_snap->ps.weaponTime / 10 + RELOAD_TIMER_X, RELOAD_TIMER_Y, 0.19f, relStr, colorYellow,qfalse, qtrue, &eth.fontVariable);
        ...
        drawText (WARNING_X, WARNING_Y, 0.29f, "Reload", fxphase.color[fxphase.clips_phase],qtrue, qfalse, &eth.fontVariable);
    }
    ...
    if (isAimableWeapon()) {
    	if (clips < 5 ) {
	    	if (ammo > 0 ) {  // Immortal Add this fature to menu can be turn on/off
				char wtext[100];
				if ((seth.value[VAR_RELOAD_NEED]) && !eth.reloading && health > 0) {
	...
	//draw health icon 
	if (seth.value[VAR_EXTRA_VISUALS]) {
	syscall_CG_R_SetColor(colorBlue);
	if (health > 40)
	    drawPic(H_X, H_Y, H_W, H_H, eth.weaponIcons[WP_MEDKIT]);

	// draw health value 
	if ((seth.value[VAR_LOW_HP]) && health < 40 && health > 0) {
