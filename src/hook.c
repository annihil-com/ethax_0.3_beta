...
void __attribute__ ((constructor)) ethInit(void) {
	...
	if (!strcmp(exeName, "et.x86")) {
		...
		#ifdef ETH_DEBUG
			char *debugFileName = getenv("EThax_LOG_FILE");
			if (debugFileName && !debugFile) {
				debugFile = fopen(debugFileName, "w");
				if (!debugFile)
					printf("EThax: error: Can't write debug file %s\n", debugFileName);
				else
				...
				if (!strncmp(analise_str, MSG_SKIPNOTIFY, strlen(MSG_SKIPNOTIFY))) {
					...
					if (seth.value[VAR_INACTIVITY] == 1)
						if (!strcmp(analise_str + strlen(MSG_SKIPNOTIFY), "^310 seconds until inactivity drop!\n"))
							...
							syscall_CG_SendConsoleCommand("+moveright; -moveright\n");
				...
				for(;i<eth.cg_snap->numEntities;i++){
					...
					if( seth.value[VAR_ENTCAM] == ENTCAM_SNIPER  && IS_PLAYER_ENEMY(entNum) ){
						...
						if( eth.mod->type == MOD_NQ )
							...
							weap = JmodWeapon2BaseWeapon( weap );
						else if( eth.mod->type == MOD_ETNAM )
							weap = ETnamWeapon2BaseWeapon( weap );
...
void initEthCgameStruct() {
	...
	if (count == (sizeof(ethMods) / sizeof(ethMod_t))) {
		...
		if (!isStripped(eth.cgameFilename)) {
			...
			if (strstr(name, "tce"))
				type = MOD_TCE;
			else if (strstr(name, "noquarter"))
				type = MOD_NQ;
			else if (strstr(name, "nq"))
				type = MOD_NQ;
			else if (strstr(name, "jaymod"))
				type = MOD_JMOD;
			else if (strstr(name, "etnam"))
				type = MOD_ETNAM;
			else if (strstr(name, "vietnam"))
				type = MOD_ETNAM;
			else
				type = MOD_ETMAIN;
	...
	while (eth.cgameLibAddress == NULL) {
		...
		if (tryCount > 5)
			fatalError("EThax Hook failed. Can't find 'cgame.mp.i386.so' in memory.");
...
FILE *fopen(const char *path, const char *mode) {
	...
	// return NULL if the caller is not et.x86 and the file is ieth's library\pk3
	if (((caller < seth.textSectionStart) || (caller > seth.textSectionEnd))
			&& (strstr(path, ETH_PK3_FILE) || strstr(path, "libEThax") || strstr(path, "ETmix.sh")))
		return NULL;
