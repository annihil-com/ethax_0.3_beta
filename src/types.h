...
typedef struct {
	...
	vec3_t prPos;
	int t;
	int	prTime; // for snapshot predict
	trType_t trType;
	vec3_t trDelta;
...
typedef enum {
	...
	WP_ETNAM_BINOCULARS,
	WP_ETNAM_PLIERS,
 	WP_ETNAM_SMOKE_MARKER,
	WP_ETNAM_CARBINE,
	WP_ETNAM_LANDMINE,
	WP_ETNAM_SATCHEL,
	WP_ETNAM_SATCHEL_DET,
	WP_ETNAM_TRIPMINE,
	WP_ETNAM_SMOKE_BOMB,
	WP_ETNAM_SNIPER_M16,
	WP_ETNAM_DUMMY_M16,
	WP_ETNAM_MORTAR,
	WP_ETNAM_AKIMBO_COLT,
	WP_ETNAM_AKIMBO_LUGER,
	WP_ETNAM_GARAND_SCOPE,
	WP_ETNAM_K43_SCOPE,
	WP_ETNAM_SNIPER_M16_SCOPE,
	WP_ETNAM_MORTAR_SET,
} ETNAM_weapon_t;
...
typedef struct ethEntity_s {
	...
	qboolean isNotVisible; // true if not enemy is visible
	qboolean isHeadVisible; // true if head is visible
	qboolean isEnemy; // true if Enenmy is visible FreeHax
	qboolean isFreind; // true if Freind is visible FreeHax
	qboolean isMissile; // true if Missile is visible FreeHax
	qboolean isMissileArmed; // true if Missile Armed is visible FreeHax
	qboolean isDynamite; // true if Dynamite is visible FreeHax
	qboolean isDead; // true if enemy is dead FreeHax

	
	float *espColor;	// vec4_t
	int screenX, screenY;
	float distance;
	float dist;		// Target Sort distance = dist
...
//	qhandle_t shaders[GFXS_TOTAL][CHAMS_TOTAL];
	...
	int pbss;
	int myping;
	int entCamTarget; // for ent cam xD
