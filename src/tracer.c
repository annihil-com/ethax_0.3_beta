...
{
	if ((seth.value[VAR_TRACE]) && seth.value[VAR_TRACE_SELECTOR] == TRACE_DISTANCE);
...
int free_trace_test(const void *elm1, const void *elm2)
{
	if ((seth.value[VAR_TRACE]) && seth.value[VAR_TRACE_SELECTOR] == TRACE_FREE);
	const trace_point *a = elm1;
	const trace_point *b = elm2;
...
{
	if ((seth.value[VAR_TRACE]) && seth.value[VAR_TRACE_SELECTOR] == TRACE_ANGLE);
	const trace_point *a = elm1;
	const trace_point *b = elm2;
...
#define MAX_TR	16
	...
	if (trtype !=ANGLE_TRACE_BODY){
		...
		if( tr.fraction == 1.f ){
			...
			return qtrue;
		} else if( trtype == ANGLE_TRACE_CENTER )
			return qfalse;
	}
	...
	switch( trtype ){
		...
		case ANGLE_TRACE_RANDOM_SURFACE: {
		...
		case ANGLE_TRACE_RANDOM_VOLUME: {
		...
		case ANGLE_TRACE_CAPSULE_VOLUME: {
			float phi, r;
		...
		// for capsules size[0] is the radious and size[2] the height
		case ANGLE_TRACE_CAPSULE_SURFACE: {
			// trace random points within capsule volume
			vec3_t dir, dr;
			float phi;
...
//just won't notice it's change so reconnect or smtg
	...
	if( speed < 0 )
		...
		VectorCopy( hbox->size, size );
		size[0] += seth.value[VAR_BOXADD]*speed/eth.cg_snap->ps.speed;
		size[1] += seth.value[VAR_BOXADD]*speed/eth.cg_snap->ps.speed;
		size[2] += seth.value[VAR_BOXADD]*speed/eth.cg_snap->ps.speed;
	}
	...
	*/
	if( trace_box( p, size, eth.muzzle, player, IS_PLAYER_ENEMY(player) ? ANGLE_TRACE_RANDOM_SURFACE : ANGLE_TRACE_CENTER, hit ) ){
	...
	if(  trace_box( cv, size, eth.muzzle, player, ANGLE_TRACE_BODY, hitpoint ) ){
		return qtrue;
	} else
		return qfalse;
	}
